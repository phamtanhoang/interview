...
- Spread (`...`): "trải" phần tử ra khi tạo mảng/object/gọi hàm.
- Rest (`...`): gom phần còn lại vào một biến (tham số hàm hoặc khi destructuring).
- Mẫu "props passthrough" (`{...rest}`) hữu ích cho component tái sử dụng. Bẫy: spread chỉ là shallow copy; thứ tự quan trọng (`{...defaults, ...overrides}` → overrides thắng).
...
- ESM là hệ module chuẩn của JS (`import`/`export`); phân tích phụ thuộc tĩnh lúc biên dịch → cho phép tree-shaking. Khác CommonJS (`require`).
- Named export: nhiều thứ/file, import đúng tên. Default export: một thứ chính/file, import đặt tên tùy ý.
- `import` tĩnh: giải quyết lúc bundle, nằm đầu file. `import()` động: trả Promise, tải khi cần lúc chạy → tạo chunk riêng (code-splitting).
- Next.js có `next/dynamic` (kèm `ssr: false`). Bẫy: default export khó tree-shake và refactor tên hơn → nhiều team ưu tiên named export.
...
- `key` cho React định danh **ổn định** để map đúng phần tử giữa hai lần render → tái dùng đúng node DOM và đúng state.
- `key` chỉ cần **duy nhất giữa các siblings** (thường là id từ dữ liệu); không cần unique toàn cục và không đọc được như prop.
- **Không dùng index** vì khi sắp xếp lại/chèn/xóa ở giữa, index bị "dịch" → React map sai → state nội bộ hoặc DOM bị gán nhầm dòng.
- Index tạm chấp nhận khi danh sách **tĩnh**, không reorder, không state nội bộ. Tránh `Math.random()` (đổi mỗi render → remount cả list).
...
| | `props` | `state` |
|---|---|---|
| Sở hữu bởi | Cha truyền xuống | Bản thân component |
| Thay đổi? | Bất biến (read-only) với con nhận | Đổi qua setter (`useState`/`setState`) |
| Vai trò | Dữ liệu/cấu hình đầu vào | Dữ liệu nội bộ đổi theo thời gian |

- Cách nhớ: **props giống tham số hàm** (ngoài truyền vào, trong không tự sửa); **state giống biến cục bộ ghi nhớ được** qua các lần render.
- Cả hai khi đổi đều kích hoạt re-render.
- Bẫy: đừng "sửa props" trong con. Con cần đổi dữ liệu cha → cha truyền callback (vd `onChange`) để con báo lên, cha mới cập nhật state.
...
- Dữ liệu chảy **một chiều từ trên xuống**: cha → con qua props; con không sửa trực tiếp dữ liệu cha.
- Con cần đổi → gọi **callback do cha truyền xuống**; cha cập nhật state; state mới lại chảy xuống. Nhờ vậy luồng dễ dự đoán, dễ debug (lần ngược lên tìm nơi giữ state).
- Khác two-way binding của framework cũ. "Two-way" trong React (input controlled) thực chất vẫn một chiều: `value` đi xuống, `setState` đi lên — chỉ là tự nối thủ công.
- Nhiều component cần chung state → "nâng state lên" (lifting state up) tổ tiên chung gần nhất; quá nhiều tầng (prop drilling) → dùng Context hoặc Redux Toolkit/Zustand.
...
- Component chỉ trả về **một node gốc**; `Fragment` nhóm nhiều con mà **không thêm node DOM thừa** (không tạo `<div>` bao ngoài).
- Quan trọng với layout flex/grid, hoặc HTML hợp lệ không cho thẻ bọc (`<td>` trong `<tr>`, `<option>` trong `<select>`).
- Hai dạng: rút gọn `<>...</>` (**không nhận key/prop**); đầy đủ `<React.Fragment key={...}>` (dùng khi render danh sách cần `key`).

```jsx
{items.map((item) => (
  <React.Fragment key={item.id}>
    <dt>{item.term}</dt><dd>{item.desc}</dd>
  </React.Fragment>
))}
```
...
- **Render danh sách**: dùng `Array.prototype.map` biến mảng dữ liệu thành mảng element, mỗi phần tử cần `key` ổn định.
- **Render có điều kiện** (JSX chỉ nhận biểu thức, không nhận `if`): toán tử ba ngôi `cond ? A : B`; toán tử `&&` để hiện/ẩn; hoặc tách biến/hàm ở trên `return` cho logic phức tạp.
- Bẫy kinh điển với `&&`: vế trái là **số 0** sẽ render ra `0` (0 là falsy nhưng vẫn là giá trị render được); `false`/`null`/`undefined` thì không render gì.

```jsx
{isLoading ? <Spinner /> : <Content data={data} />}
{items.length > 0 && <List items={items} />} // ĐÚNG: ép boolean rõ ràng
```
...
- `useMemo(() => compute(), deps)`: ghi nhớ KẾT QUẢ phép tính. `useCallback(fn, deps)`: ghi nhớ chính HÀM (= `useMemo(() => fn, deps)`).
- Nên dùng: phép tính thực sự nặng, hoặc giữ tham chiếu ổn định để truyền làm prop/dependency cho component con đã `React.memo`.
- Lạm dụng có hại: bản thân memo tốn chi phí (cache + so sánh deps), có thể lớn hơn cái tiết kiệm; code rối, dễ sai deps; nếu con không `memo` thì `useCallback` vô nghĩa.
- React 19 + React Compiler tự động memoize, giảm nhu cầu viết tay.

```tsx
const filtered = useMemo(() => list.filter(i => i.name.includes(q)), [list, q]);
...
- Cho phép đọc giá trị từ `Provider` ở trên cây, tránh prop drilling; hay dùng cho theme, ngôn ngữ, user.
- Nhược điểm: khi `value` đổi, MỌI component dùng context đó đều re-render — không có selector như Redux/Zustand.
- Lưu ý: đừng đặt giá trị đổi liên tục vào context dùng rộng; nếu `value` là object/array thì bọc `useMemo`; server-state dùng TanStack Query, global state phức tạp dùng Zustand/Redux Toolkit.

```tsx
const theme = useContext(ThemeContext); // không cần truyền prop qua tầng trung gian
```
...
- Cả hai thuộc concurrent React, đánh dấu cập nhật là "không khẩn cấp" để UI luôn phản hồi.
- `useTransition`: bọc HÀNH ĐỘNG `setState` vào `startTransition`, trả về `isPending`. Dùng khi bạn KIỂM SOÁT chỗ gọi `setState`.
- `useDeferredValue`: bọc GIÁ TRỊ, trả về phiên bản "trễ". Dùng khi chỉ nhận được giá trị (ví dụ từ prop) mà không kiểm soát nơi set.
- Cả hai không "tăng tốc" tính toán, chỉ đổi ĐỘ ƯU TIÊN.

```tsx
const [isPending, startTransition] = useTransition();
setQuery(value);                              // khẩn cấp -> input mượt
startTransition(() => setResults(filter(value))); // hạ ưu tiên
```
...
- Hook để subscribe một external store (store tự viết, `localStorage`, `navigator.onLine`) an toàn với concurrent rendering. Nhận `subscribe`, `getSnapshot` (client) và `getServerSnapshot` tùy chọn (SSR).
- Quan trọng: tránh "tearing" (UI thấy giá trị không nhất quán); đảm bảo mọi component đọc cùng snapshot. Là API mà Zustand, Redux dùng bên dưới.
- Lưu ý SSR: phải có `getServerSnapshot` trả giá trị mặc định, nếu không sẽ hydration mismatch. Đa số app không gọi trực tiếp — dành cho tác giả thư viện.
...
- Mỗi render, object/array/arrow inline tạo **reference mới** -> phá so sánh nông của `React.memo`; làm `useEffect`/`useMemo`/`useCallback` nhận dependency "mới" chạy lại thừa.
- **Quan trọng khi:** truyền xuống component đã `React.memo`, làm dependency hook, hoặc trong vòng lặp render nhiều item.
- **Không đáng lo khi:** component nhận *không* memo và rẻ (vốn đã render theo cha). Chi phí GC tạo object nhỏ là cực rẻ; chi phí thật là re-render dây chuyền.
- Bẫy: tối ưu sớm, bọc mọi handler bằng `useCallback`. React Compiler (React 19) khi bật tự lo phần lớn.
...
- Redux/Zustand: **top-down / store tập trung**, component đọc một lát cắt.
- Jotai/Recoil: **bottom-up / atomic**, state chia thành nhiều **atom** nhỏ độc lập; component đăng ký đúng atom cần. Gần với `useState` nhưng chia sẻ được và **derive** được.
- Ưu điểm: chỉ component dùng atom đó mới re-render; **derived atom** tính state phái sinh mà không lưu trùng.
- Recoil (Facebook) gần như **không còn phát triển tích cực** -> dự án mới **ưu tiên Jotai**.
- Chọn atomic khi state phân mảnh, nhiều mảnh nhỏ liên quan; app ít state toàn cục đơn giản thì Zustand gọn hơn.

```ts
const countAtom = atom(0);
const doubledAtom = atom((get) => get(countAtom) * 2); // derived, không lưu trùng
```
...
- `<Switch>` (match đầu tiên) → `<Routes>` (chọn route phù hợp nhất, có ranking).
- Render: bỏ `component`/`render`, luôn dùng `element={<Comp />}`.
- Hỗ trợ **nested routes** + `<Outlet />`; route con dùng relative path.
- `useHistory()` → `useNavigate()`; không cần `exact` (mặc định match chính xác).

```tsx
<Routes>
  <Route path="/" element={<Layout />}>      {/* Layout phải có <Outlet /> */}
...
- **Bẫy:** quên `<Outlet />` ở cha → route con không hiện; đặt `/` đầu path con biến nó thành đường dẫn tuyệt đối.
...
- **`useParams()`**: đọc tham số động `:id` trong path; giá trị luôn là **string**.
- **`useSearchParams()`**: đọc/ghi query string, trả `[searchParams, setSearchParams]` (như `useState`); `searchParams` là `URLSearchParams`.
- **`useLocation()`**: object location hiện tại (`pathname`, `search`, `hash`, `state`).

```tsx
const { category } = useParams();
const [sp, setSp] = useSearchParams();
const page = Number(sp.get('page') ?? 1);
setSp({ page: String(page + 1) });   // ghi đè TOÀN BỘ query
```

- **Bẫy:** params/search đều là string (phải `Number(...)`); `setSearchParams` ghi đè hết — muốn giữ param khác phải merge thủ công.
...
- **`clsx`/`classnames`**: nối class **có điều kiện** gọn (string/object/array, bỏ giá trị falsy).
- **`cva`**: định nghĩa **variant** có cấu trúc (size, intent...) + suy ra type cho props; hợp xây UI component nhiều biến thể (shadcn/ui dùng).
- **`tailwind-merge`** (`twMerge`): xử lý xung đột class trùng nhóm (`px-2` + `px-4` → giữ cái sau).

```tsx
clsx('btn', { 'btn--active': isActive, 'opacity-50': disabled });
const button = cva('rounded', {
  variants: { intent: { primary: 'bg-blue-600', ghost: 'bg-transparent' } },
});
twMerge(button({ intent }), className);   // override an toàn
```

- Bộ ba này là nền tảng cách shadcn/ui tổ chức component.
...
- Thư viện animation **khai báo**: mô tả trạng thái, thư viện lo chuyển động (spring/tween).
- **`motion.*`**: phần tử animate được; **`initial`/`animate`/`exit`**: lúc mount/hiển thị/unmount; **`transition`**: kiểu chuyển động; **`variants`**: gom trạng thái + `staggerChildren`.
- **`AnimatePresence`**: cho phép chạy animation khi component **bị unmount** — điều React không hỗ trợ sẵn (bình thường biến mất tức thì).

```tsx
<AnimatePresence>
  {show && (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      exit={{ opacity: 0 }} />   // exit chạy được nhờ AnimatePresence
  )}
</AnimatePresence>
```

- **Bẫy:** quên `AnimatePresence` → `exit` không chạy; danh sách phải có **`key` ổn định** để Motion theo dõi item bị gỡ.
...
- Công cụ phát triển + tài liệu hóa component **độc lập** (isolation) — chạy từng component ở mọi trạng thái mà không cần khởi động cả app.
- Mỗi "story" = một trạng thái (loading/error/empty/có data). Lợi ích:
  - Phát triển isolation, không phụ thuộc backend/state thật.
  - **Tài liệu sống** cho design system (dev + designer cùng xem).
  - **Visual/regression testing** (Chromatic); test tương tác + a11y (addon, play function).
...
- Đặc biệt giá trị cho team xây **component library / design system**; cầu nối designer - dev - QA.
...
- `react-i18next` (trên `i18next`): quản lý resource dịch theo namespace, đổi ngôn ngữ runtime, hook `useTranslation` (`t`, `i18n.changeLanguage`).
- **Interpolation/plural**: dùng `{{var}}` và `_one`/`_other` thay vì ghép chuỗi.
- **Namespace + lazy load**: tách file dịch theo trang/feature để giảm bundle.
- **Số/ngày/tiền**: dùng `Intl` theo locale, không hardcode.
- **Next.js App Router**: react-i18next chạy tốt phía client; **Server Components** thường dùng `next-intl`; cần chiến lược locale routing (`/en`, `/vi`).

```tsx
const { t, i18n } = useTranslation();
<p>{t('greeting', { name })}</p>           // "Xin chào, {{name}}"
<p>{t('cart.items', { count })}</p>        // plural tự động
```

- **Tránh:** một file dịch khổng lồ; dịch bằng nối chuỗi (sai ngữ pháp nhiều ngôn ngữ).
...
- Thư viện **headless** xây bảng phức tạp: sorting, filtering, pagination, grouping, row selection, virtualization.
- Lo toàn bộ **logic + state** nhưng **không render markup/style** — bạn tự render `<table>` và tự style.
- Lợi: tự do giao diện (khác AG Grid/MUI DataGrid), nhẹ, framework-agnostic.
...
// render bằng table.getHeaderGroups() / getRowModel() + flexRender(...)
```

- **Khi nào:** cần bảng tùy biến cao + kiểm soát logic; bảng cực lớn kết hợp **TanStack Virtual**. Muốn grid đóng gói sẵn UI/enterprise thì AG Grid / MUI DataGrid hợp hơn.
...
- Box Model: mỗi phần tử là hộp gồm `content` → `padding` → `border` → `margin` (từ trong ra ngoài).
- `content-box` (mặc định): `width` chỉ tính content; rộng thực = `width + padding + border` → khó tính layout.
- `border-box`: `width` đã gồm `padding` + `border`, nên đặt `width: 200px` là luôn rộng 200px.
- React/Next.js reset toàn cục để layout dễ đoán khi ghép nhiều component có padding/border khác nhau.
- Bẫy: `<input width:100%>` + padding với `content-box` sẽ tràn ra ngoài. `margin` không nằm trong `box-sizing`, luôn cộng thêm ở ngoài.
...
- **Flexbox**: bố cục một chiều (một hàng HOẶC một cột); tư duy content-first; hợp navbar, toolbar, căn giữa.
- **Grid**: bố cục hai chiều (hàng + cột); tư duy layout-first; hợp dashboard, card gallery, layout trang.
- Thực tế kết hợp: Grid dựng khung tổng, Flexbox sắp xếp nội dung trong từng ô.

```tsx
// Toolbar (Flex, 1 chiều)
<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
  <Logo /><SearchBox style={{ flex: 1 }} /><UserMenu />
</div>

// CardGrid (Grid, 2 chiều, tự xuống dòng)
...
- Tính theo bộ ba `(a, b, c)`: `a`=ID, `b`=class/attribute/pseudo-class, `c`=element/pseudo-element. So từ trái sang.
- Inline style cao hơn mọi selector; `!important` ghi đè cả inline. Specificity bằng nhau → rule khai báo **sau** thắng.
- Tránh `!important`: phá thứ tự tự nhiên, khó ghi đè về sau, gây "cuộc đua important", khó bảo trì.
- **Tailwind**: utility đều `(0,1,0)` nên thứ tự build quyết định; dùng `tailwind-merge` (`clsx`/`cn`) để ghi đè đúng thay vì `!important`.
- **styled-components**: class hash riêng nên ít xung đột; ghi đè con bằng `&&` (nhân đôi class) thay cho `!important`.

```css
p        { color: black; }  /* (0,0,1) */
p.intro  { color: blue;  }  /* (0,1,1) thắng */
#main    { color: green; }  /* (1,0,0) thắng cả hai */
```
...
- CRP: chuỗi bước biến HTML/CSS/JS thành pixel. Các bước: Parse HTML→DOM → Parse CSS→CSSOM → Render Tree (ghép, bỏ `display:none`) → Layout/Reflow → Paint → Composite.
- CSS render-blocking: trình duyệt chờ CSSOM hoàn tất mới render để tránh vẽ rồi nhảy style (FOUC).
- JS đồng bộ (`<script>` không `defer/async`) là parser-blocking — chặn cả dựng DOM.
- CRP ảnh hưởng trực tiếp FCP/LCP.
- Next.js giảm CRP: SSR/SSG, code-splitting tự động, `next/font` (tránh chặn font + giảm layout shift), `next/script` (`afterInteractive`/`lazyOnload`), inline critical CSS.
...
- **Margin collapsing**: margin dọc (top/bottom) của block kề/lồng nhau "gộp" thành margin lớn nhất, không cộng dồn. Vd `<p>` margin-bottom 20px trên `<p>` margin-top 16px → cách 20px (không phải 36px).
- Chỉ xảy ra với margin dọc, **không** trong Flex/Grid → nên dùng `gap` để kiểm soát khoảng cách. Chặn được bằng `padding`, `border`, hoặc tạo BFC (`overflow: hidden`, `display: flow-root`).
- React dùng nhiều cách 1 và 2 (Tailwind: `flex items-center justify-center` hoặc `grid place-items-center`).

```css
.parent { display: flex; align-items: center; justify-content: center; } /* 1 */
.parent { display: grid; place-items: center; }                          /* 2 */
.child  { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); } /* 3 */
.child  { width: 200px; margin-inline: auto; } /* 4: chỉ căn ngang */
```
...
- **MFE**: chia app lớn thành nhiều phần độc lập, mỗi team sở hữu và **deploy riêng**; mục tiêu là **độc lập tổ chức/deploy**, không phải hiệu năng.
- **Nên dùng** khi: nhiều team lớn cần release độc lập; nghiệp vụ tách bạch (Checkout/Search/Account); cần mix tech/version (migrate dần).
- **KHÔNG nên** khi: team nhỏ (<15-20 dev) → chi phí > lợi ích, monorepo modular đủ; yêu cầu hiệu năng cực cao (dễ tải trùng lib).

| Cách | Cơ chế | Nhược |
|---|---|---|
| **Module Federation** | Tải remote runtime, chia sẻ dep (React singleton) | Phải đồng bộ version shared dep |
| **Route-based / app shell** | Mỗi MFE 1 app, ghép qua proxy theo path | Chuyển route thường full reload |
| **iframe** | Nhúng qua `<iframe>` | Khó share state, SEO/UX kém |

Next.js có **Multi-Zones** (MFE route-based "chính chủ").

**Bẫy**: tải trùng React → "multiple React instances"; share state cross-MFE thành coupling ngầm (nên giao tiếp qua event/URL/contract); design system vênh nhau nếu không có library chung.
...
- **Design tokens**: quyết định thiết kế lưu thành **biến có tên, độc lập nền tảng** (màu, spacing, font...); tham chiếu `color.action.primary` thay vì hardcode hex. Là **nguồn chân lý duy nhất** nối Figma ↔ code.
- **Tầng token**: 1) Primitive (`blue-500`) → 2) Semantic (`color.action.primary` = `{blue-500}`) → 3) Component (`button.bg`). Lớp **semantic** là chìa khoá multi-theme: đổi theme = đổi ánh xạ semantic → primitive.
- **Đa nền tảng**: dùng chuẩn **W3C Design Tokens** (`tokens.json`) + **Style Dictionary** build ra CSS vars / JS / Tailwind / iOS/Android cùng lúc.

```css
:root { --color-action-primary: #3b82f6; }
...
**Bẫy**: bỏ lớp semantic (dùng thẳng `blue-500`) → dark mode phải sửa từng component; đặt tên theo **giá trị** (`color-blue`) thay vì **ý nghĩa** → mất tác dụng khi rebrand.
...
- **Unit test**: một đơn vị **cô lập** (hàm thuần, reducer, hook tính toán), mock hết phụ thuộc.
- **Integration test**: **nhiều phần phối hợp** (form + input + submit + API mock qua MSW + hiển thị kết quả) như luồng user thật — cho giá trị cao nhất trong React.
- Ranh giới không cứng; mấu chốt là render component thật và tương tác qua hành vi.
- **Bẫy**: tách thành nhiều unit test rời ("input nhận giá trị"...) thì test được code nhưng không chắc cả luồng chạy.

```tsx
test('đăng nhập thành công thì hiển thị lời chào', async () => {
  server.use(http.post('/api/login', () => HttpResponse.json({ token: 'abc', name: 'An' })));
  render(<LoginForm />);
...
- Currying biến `f(a,b,c)` thành chuỗi gọi từng phần `f(a)(b)(c)` hoặc `f(a,b)(c)`; dựa vào `fn.length` để biết đã đủ tham số chưa.
- Trong React: tạo handler theo tham số gọn, ví dụ `onClick={handleSelect(id)}` với `handleSelect = (id) => (e) => {...}`.
- Bẫy: `fn.length` không đếm tham số mặc định/rest -> curry generic sai với các hàm đó; dạng `handleSelect(id)` tạo hàm mới mỗi render -> phá tối ưu nếu truyền xuống component đã `memo`.
...
// curry(sum)(1)(2)(3) === curry(sum)(1, 2)(3)
```
...
- Quản lý mảng/Set id đang mở; `type="single"` (mở 1, click khác thì đóng cái cũ) hoặc `"multiple"` (mở nhiều cùng lúc).
- Animation: `height: auto` không transition CSS thuần -> dùng `grid-template-rows: 0fr->1fr`, `max-height`, đo height bằng JS, hoặc **Framer Motion** (`AnimatePresence` + `animate={{ height }}`).
- Accessibility: nút dùng `aria-expanded` + `aria-controls` trỏ panel; panel `role="region"`; dùng `hidden` (không chỉ ẩn CSS) để screen reader bỏ qua. Đừng unmount nếu cần animation đóng.
...
Kỹ năng cốt lõi: **biến cái mơ hồ thành cái đo được** trước khi viết code.

- **Hỏi làm rõ mục tiêu**: "đẹp" với ai, để làm gì? "nhanh" là LCP dưới mấy giây hay cảm giác mượt khi tương tác?
- **Biến thành acceptance criteria**: "đẹp" → theo design system; "nhanh" → LCP < 2.5s, CLS gần 0.
- **Làm rõ phạm vi & ưu tiên**: dữ liệu từ đâu, có real-time không, thiết bị mục tiêu.
- **Đề xuất MVP** rồi xác nhận bằng wireframe/prototype trước khi code thật.
- **Ghi lại thống nhất** để tránh tranh cãi sau.

Đây là dạng câu hỏi hành vi, có thể trả lời theo cấu trúc dưới đây.
...
- **Situation**: "PM giao tôi làm dashboard với mô tả rất ngắn 'cần trực quan và load nhanh', không có spec."
- **Task**: "Vừa làm rõ yêu cầu vừa giao đúng thứ người dùng cần, tránh làm xong phải đập đi làm lại."
- **Action**: "Tôi đặt buổi 30 phút với PM và designer, hỏi 3 nhóm: ai dùng và ra quyết định gì, 'nhanh' đo bằng chỉ số nào, dữ liệu real-time hay theo phút. Chốt: LCP < 2.5s, đúng design system, ưu tiên 3 widget chính, rồi dựng prototype xác nhận trước khi code."
- **Result**: "Bản đầu được duyệt gần như ngay, tránh một vòng làm lại; trang đạt LCP ~1.8s nhờ ưu tiên widget chính và lazy-load phần còn lại."
...
- STAR là khung kể chuyện giúp câu trả lời rõ ràng, có trọng tâm, chứng minh tác động thực tế.
- Nhà tuyển dụng dùng câu hỏi hành vi vì hành vi quá khứ là chỉ báo tốt nhất cho tương lai.
- **S (Situation):** bối cảnh, dự án, vai trò — ngắn gọn.
- **T (Task):** vấn đề/mục tiêu cụ thể cần giải quyết.
- **A (Action):** việc *bạn* làm (dùng "tôi", không "chúng tôi") — phần quan trọng nhất.
- **R (Result):** kết quả đo được (ưu tiên con số) + bài học.

| Phần | Mẫu (frontend) |
| --- | --- |
| S | "Dashboard React load >6s, khách phàn nàn." |
| T | "Tôi được giao kéo LCP xuống <2.5s." |
| A | "`React.lazy`+`Suspense` code-split, `next/image`, memo bảng nặng." |
| R | "LCP còn 1.8s, bounce giảm 18%." |

**Bẫy:** kể lể phần Situation, quên Result (thiếu con số), dùng "chúng tôi" suốt. Chuẩn bị sẵn 4-5 câu chuyện STAR tái sử dụng được.
...
- Câu này kiểm tra sự bình tĩnh, ưu tiên giảm thiệt hại trước root cause, và tinh thần trách nhiệm. Đừng đổ lỗi.
- **S:** "Deploy chiều thứ Sáu khiến trang thanh toán trắng màn hình trên Safari."
- **T:** "Là người trực release, tôi phải khôi phục UI nhanh nhất rồi mới điều tra."
- **A:** "*Rollback* ngay để dừng thiệt hại → mở Sentry thấy `Cannot read properties of undefined` (field API `null`, optional chaining sai chỗ) → thêm guard + `ErrorBoundary` bọc luồng thanh toán + viết test cho ca `null`."
- **R:** "UI khôi phục ~8 phút nhờ rollback, fix thật deploy sau 1 tiếng. Viết postmortem không đổ lỗi, thêm `ErrorBoundary` cho luồng quan trọng."
- **Nhấn:** mitigation (rollback/feature flag) trước root cause; dùng monitoring (Sentry); thêm test hồi quy; biến sự cố thành cải tiến hệ thống.
...
- Câu này kiểm tra tư duy trade-off và giao tiếp với người không chuyên — quyết định dựa trên **tác động kinh doanh & rủi ro**, không phải sở thích.
...
- **"Đóng thuế" (boy scout rule):** dành ~15-20% mỗi sprint trả nợ, hoặc refactor từng phần trên đường làm feature.
- **Để stakeholder quyết định có thông tin:** trình bày rõ đánh đổi, họ chọn cuối cùng.
...
**Bẫy:** trả lời cực đoan ("luôn ưu tiên feature" = thiếu trách nhiệm dài hạn; "luôn refactor trước" = thiếu nhạy bén kinh doanh). Câu tốt nằm ở giữa, tùy rủi ro và tác động.
...
- Nhà tuyển dụng muốn thấy bạn xử lý bất đồng dựa trên *dữ liệu* và *mục tiêu chung*, không phải cái tôi. Frontend ở giữa designer và backend nên kỹ năng này rất quan trọng.

Xung đột với designer:
- **S:** "Designer làm animation phức tạp cho list 200+ item bằng Framer Motion layout animation."
- **A:** "Không nói thẳng 'không làm được'. Tôi quay demo độ giật trên thiết bị thật, đề xuất: chỉ animate khi vào viewport, tôn trọng `prefers-reduced-motion`, dùng transform thay vì animate layout; mời designer tinh chỉnh cùng."
- **R:** "Giữ 90% cảm giác thiết kế mà đạt 60fps; sau đó designer chủ động hỏi về giới hạn hiệu năng từ đầu."

Xung đột với backend (hình dạng API):
- **A:** "Đề xuất 2 phương án để backend chọn: trả về dạng cây (đúng domain) hoặc tôi tự build cây client; viết contract (TypeScript/OpenAPI) thống nhất trước khi code."
- **R:** "Chọn backend trả cây vì tái dùng cho mobile; thống nhất contract trước giảm hẳn vòng đi-về."
- **Nhấn:** dùng bằng chứng (demo, số đo, contract) thay vì cảm tính; quy về mục tiêu chung; đưa nhiều phương án để bên kia chọn.
...
- Nguyên tắc cốt lõi: **nói về tác động (impact), không nói cơ chế (mechanism)** — họ quan tâm ảnh hưởng đến người dùng/tiền/thời gian, không phải `useMemo`.
- **Dùng ẩn dụ:** vd hydration = "ảnh in sẵn để thấy ngay, rồi JS chạy nền thổi hồn cho nút bấm hoạt động".
- **Quy sang ngôn ngữ kinh doanh:** thay vì "chưa code-split" → "tải chậm 3s, mỗi giây mất X% người mua".
- **Đưa đánh đổi dạng lựa chọn:** "A nhanh ra mắt nhưng tốn bảo trì; B ngược lại; tôi đề xuất A vì..."
- **Trực quan hóa:** biểu đồ, demo, con số trước/sau.

Ví dụ thuyết phục PM chuyển CSR → SSR/SSG:

> ❌ "App đang CSR nên FCP/LCP tệ, ta nên dùng `getStaticProps`."
>
> ✅ "Người dùng nhìn màn hình trắng ~4s, Google chấm tốc độ thấp ảnh hưởng thứ hạng. Có kỹ thuật tạo sẵn trang để thấy nội dung gần như tức thì và SEO tốt hơn; tôi cần ~3 ngày."

**Bẫy:** dùng jargon khiến họ gật gù nhưng quyết sai; đi quá sâu chi tiết. Kiểm tra lại bằng "anh/chị thấy rõ không?".
...
- Câu này đo độ trưởng thành cảm xúc và growth mindset. Đừng chọn feedback "giả vờ tiêu cực" kiểu "tôi quá cầu toàn"; chọn feedback thật cho thấy bạn đã thay đổi.
- **S:** "Code review: senior nhận xét PR của tôi gom quá nhiều (component mới + refactor + đổi state management), khó review, rủi ro cao."
- **T:** "Ban đầu hơi tự ái, nhưng tự nhắc feedback nhắm vào *code*, không nhắm vào *tôi*."
- **A:** "Cảm ơn, hỏi lại kỳ vọng, tách thành 3 PR nhỏ độc lập; hỏi senior về quy ước PR của team; tập thói quen mỗi PR làm một việc + mô tả rõ."
- **R:** "PR sau merge nhanh hơn; vài tháng sau tôi đưa lời khuyên tương tự cho junior."
- **Nhấn:** thừa nhận cảm xúc ban đầu trung thực, tách cái tôi khỏi công việc, biến feedback thành hành động và thay đổi lâu dài. Tránh đổ lỗi người đưa feedback.
...
- **Nguồn chính thống:** blog/changelog React, Next.js (vercel.com/blog), TanStack — đọc release notes.
- **Theo dõi người tạo công cụ:** Dan Abramov, đội Vercel, Tanner Linsley trên X/GitHub.
- **RFC & GitHub Discussions:** hiểu *tại sao* một API ra đời (vd RSC, React Compiler).
- **Học bằng cách làm:** side project nhỏ thử công nghệ mới trước khi đề xuất vào dự án thật.
- **Newsletter/podcast:** nắm bức tranh lớn mà ít tốn thời gian.
...
- **A:** "Cùng PM phân loại MoSCoW (form đăng ký + hero = Must; animation phụ, A/B test = Could, cắt); tái dùng component design system; dùng feature flag để bật phần chưa xong sau launch; cảnh báo stakeholder ngay ngày đầu về rủi ro trễ để họ quyết cắt gì."
...
- Câu này kiểm tra trung thực, ownership, khả năng học hỏi. "Tôi chưa từng mắc lỗi nào" = thiếu tự nhận thức. Chọn lỗi thật, nhận trách nhiệm rõ, tập trung vào bài học.
- **S:** "Đẩy thay đổi lưu state lên localStorage mà không test kỹ trên chế độ riêng tư của Safari (nơi `localStorage` bị giới hạn)."
- **T:** "Một nhóm người dùng bị crash app khi mở; tôi cần thừa nhận và khắc phục."
- **A:** "Báo ngay team/stakeholder thay vì giấu, không đổ lỗi trình duyệt; bọc storage trong try/catch + fallback in-memory + test ca storage không khả dụng; đề xuất thêm kiểm thử cross-browser (cả chế độ riêng tư) vào checklist QA."
- **R:** "Xử lý trong ngày; checklist mới giúp bắt thêm vài vấn đề tương thích trước production. Bài học: giả định về môi trường người dùng là nguồn bug nguy hiểm nhất của frontend."
- **Nhấn:** chủ động báo lỗi và nhận trách nhiệm sớm; sửa cả triệu chứng lẫn quy trình; rút bài học cụ thể. Tránh chọn lỗi quá nhỏ hoặc đổ lỗi hoàn cảnh.
